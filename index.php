    ...
    <form method="post" class="kw-window">
        ...
        <div class="kw-formbody kw-tab-pane active" data-pane="application">
            ...
            <fieldset><legend><?= Support::h($t('edit.download_source')) ?></legend>
                <div class="form-group row"><label class="col-sm-3 col-form-label col-form-label-sm"><input type="radio" checked> <?= Support::h($t('edit.url')) ?></label><div class="col-sm-7"><input class="form-control form-control-sm" name="download_url_template" value="<?= Support::h($app['download_url_template']) ?>"></div><div class="col-sm-2"><a class="btn btn-sm btn-light border btn-block" href="index.php?variables=<?= (int)$app['id'] ?>"><?= Support::h($t('button.variables')) ?></a></div></div>
                <div class="form-group row"><label class="col-sm-3 col-form-label col-form-label-sm"><input type="radio"> <?= Support::h($t('edit.filehippo_id')) ?></label><div class="col-sm-9"><input class="form-control form-control-sm" disabled></div></div>
                <div class="form-group row"><label class="col-sm-3 col-form-label col-form-label-sm"><?= Support::h($t('edit.beta_versions')) ?></label><div class="col-sm-9 pt-1"><label class="mr-3"><input type="radio" name="beta_policy" value="default" <?= $app['beta_policy'] === 'default' ? 'checked' : '' ?>> <?= Support::h($t('edit.beta_default')) ?></label><label class="mr-3"><input type="radio" name="beta_policy" value="avoid" <?= $app['beta_policy'] === 'avoid' ? 'checked' : '' ?>> <?= Support::h($t('edit.beta_avoid')) ?></label><label><input type="radio" name="beta_policy" value="download" <?= $app['beta_policy'] === 'download' ? 'checked' : '' ?>> <?= Support::h($t('edit.beta_download')) ?></label></div></div>
            </fieldset>
